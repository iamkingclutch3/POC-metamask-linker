...
module.exports = {
    ...
    run: (client, message, args) => {
        ...
        if(!message.member.roles.cache.some(role => role.id === client.config.allowrole)) return message.channel.send("This command is only for whitelisted people see <#914258300827213845> for more info.").then(c => {
            if(address) message.delete()
            setTimeout(() => c.delete(), 7000)
        })
        ...
        if(client.db.getData(message.author.id) && !message.member.roles.cache.some(role => role.id === client.config.admin) && message.author.id !== "587637039051046922"){
            message.channel.send(`${message.author}, you can only use this command once. If you want to change your linked address create a ticket.`).then(a => {
                message.delete()
                setTimeout(() => a.delete(), 7000)
            })
            return
        } 
